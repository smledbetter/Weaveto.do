...
export interface AgentExports {
  ...
  init: () => void;
  /** Called when a task event occurs. Agent should call host_get_event to read the event. */
  on_task_event: () => void;
...
export interface AgentHostImports {
  ...
  /** Read the pending task event into agent memory at buf_ptr. Returns bytes written (0 if none). */

...
function makeContext(overrides?: Partial<HostContext>): HostContext {
	return {
		...
		roomId: 'test-room',
		moduleId: 'test-agent',
		prfSeed: new Uint8Array(32).fill(0x42),
		onEmitEvent: vi.fn(),
		stateCache: null,
		stateDirty: false,
		...overrides,
...
const ALL_PERMS: AgentPermission[] = ['read_tasks', 'read_members', 'emit_events', 'persist_state'];
...
describe('Host Imports: host_get_members', () => {
	it('writes members JSON to agent memory', () => {
		...
		expect(parsed.map((m: { displayName: string }) => m.displayName).sort()).toEqual([
			'Alice',
			'Bob',
		]);
	});
	...
	it('returns 0 when read_members permission is denied', () => {
		const members = new Map([['key1', { identityKey: 'key1', displayName: 'Alice' }]]);
		const ctx = makeContext({ members });
		const memory = makeMemory();
...
describe('Host Imports: host_log', () => {
	it('logs message with agent prefix', () => {
		...
		expect(consoleSpy).toHaveBeenCalledWith('[agent:my-agent]', 'Hello from agent');
		consoleSpy.mockRestore();
	});
});
